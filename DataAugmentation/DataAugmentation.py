...
class DataAugmentation:
    ...
    def scale_random_crop(self,input_image,scale_range=(350,600), crop_size=(340,340),file_name='scale_random_crop_image.bmp'):
        '''scale_rangeで指定したサイズの範囲で拡大してから、crop_sizeのサイズでランダムに画像を切り出す'''
    ...
    def gauss_noise(self,input_image,sigma=15,file_name='gauss_image.bmp'):
        '''ガウスノイズを付与する 調査中'''
        gauss_image = np.copy(input_image)
        row, col, ch = gauss_image.shape
        mean = 0
        gauss = np.random.normal(mean, sigma, (row, col, ch))
        gauss = gauss.reshape(row, col, ch)
        gauss_image = gauss_image + gauss
        skimage.io.imsave(file_name,gauss_image)
        return gauss_image

    def salt_noise(self,input_image,rate=4,file_name='salt_image.bmp'):
        '''ソルトノイズを付与する'''
    ...
    def pepper_noise(self,input_image,rate=4,file_name='pepper_image.bmp'):
        '''ペッパーノイズを付与する'''
